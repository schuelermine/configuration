{
  inputs = {
    ...
    nixpkgs-fix.url = "github:midchildan/nixpkgs/fix/noto-cjk-src";
    nix-lib.url = "github:schuelermine/nix-lib/b0";
    nix-symlink.url = "github:schuelermine/nix-symlink/b0";
  };
  outputs = { self, nixpkgs, nix-symlink, nixpkgs-fix, nix-lib }:
    with nix-lib; {
      nixosConfigurations.buggeryyacht-nixos = nixpkgs.lib.nixosSystem {
        system = "x86_64-linux";
        specialArgs = {
          nixpkgs-fix = nixpkgs-fix.legacyPackages.x86_64-linux;
